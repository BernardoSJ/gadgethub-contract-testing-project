...
test.describe('Contract: GET /gadgets and /gadgets/{id}', () => {
    ...
    test('Returns 200 for valid gadget id and matches contract schema', async ({ request }) => {

        const response = await request.get(`/gadgets/1`);
        expect(response.status()).toBe(200); //Validate response status code is 200
    ...
    test('Returns 400 when gadget id is not allowed by contract', async ({ request }) => {
